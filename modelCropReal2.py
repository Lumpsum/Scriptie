...
def generator(indices, batchSize = 1):
...
json_file = open('./Architecture/modelReal2.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
loaded_model = model_from_json(loaded_model_json)
loaded_model.load_weights("./Weights/modelReal2.h5")
print("Loaded model from disk")

optimizer = Adam(lr=1e-5)
loss = ['mean_squared_error']
metrics = ['mae', 'accuracy', test]
loaded_model.compile(optimizer=optimizer, loss=loss, metrics=metrics)

filepath = "./Weights/ModelCropReal2/weights-{epoch:02d}-{val_acc:.2f}.hdf5"
...
tensorboard = TensorBoard(log_dir='./logs/ModelCrop4', write_graph=True, write_images=True, batch_size=batchSize)
